...
import { hex2bytes, hex2str, num2hex, str2bytes } from 'utils/converter';
import { swap32 } from 'utils/endian';
...
function digest(message: string, prevHash: string, prevLength: number) {
    class FakeMD4 extends MD4 {
        ...
        iv = Uint32Array.from(chunkArray(hex2bytes(prevHash), 4).map((i) => i.reverse()).map(concatUint8));
        ...
        pad(msg: string) {
            ...
            str2bytes(message + '\x80' + '\x00'.repeat(l1 - 9 - l0))
                .concat(hex2bytes(num2hex(swap32((l0 + prevLength) * 8), 16)))
...
export function lengthExtension(key: string, newMessage: string) {
    ...
    const l1 = ~~((l0 - 56) / 64 + 2) * 64;
    const padded = originalMessage + '\x80' + '\x00'.repeat(l1 - 9 - l0) + hex2str(num2hex(swap32(l0 * 8), 16));
