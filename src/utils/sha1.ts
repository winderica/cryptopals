...
export class SHA1 {
    private readonly h = Uint32Array.from([0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0]);
    ...
    digest(message: string) {
        chunkArray(this.pad(message), 64).map((chunk) => {
            ...
            [a, b, c, d, e].map((i, j) => this.h[j] += i);
        });
        return [...this.h].map((i) => i.toString(16).padStart(8, '0')).join('');
    }

    private pad(message: string) {
